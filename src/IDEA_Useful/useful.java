...
public class useful {
    ...
    //Shift + Alt + Стрелка - Двигаем строку вверх или низ
    //Alt + Стрелка - переходим по верхним вкладкам
    //Shift + Стрелка - Стралками вправо или лево выделяем текст
    //Ctrl + [Пробел] - Idea покажет какие методы есть в классе
    //Ctrl + Shift + A - Инструмент по поиску команд или действий в IDEA
    //Ctrl + D - копирут строку на которой находитесь
    //Ctrl + E - Смотрем на последние открытые классы
    //Ctrl + Y - удаляет строку на которой находитесь
    //Ctrl + O - вызов генератора Оверайт методов
    //Ctrl + R - Replace заменяем текст который нам нужно на другой текс(это применимо ко всему)
    //Ctrl + H - Смотрем все классы и интерфейсы которые наследуют определенный клас или интерфейс
    //Ctrl + Q + установить курсор - можем смотреть документацию про элементы
    //Ctrl + Alt + L - форматирует код, чтобы боло все читабельно
    //Ctrl + Alt + O - удаляет все, что не используеться
    //Ctrl + Alt + M + выделяем_код - Создаем метод(помещаем выбраный код в метод)
    //Ctrl + Alt + T - вызов Surround With (метод для обворачивания)
    //Ctrl + f12 - смотрем на перечень всего в классе
    ...
    //F6 - можно переносить класс или выносить класс в отдельный
    //Ctrl + /, Ctrl + Shift + / - быстрое комментирование
    //Alt + Enter + курсор - можем быстро написать import для класса
    //Alt + f7 - посмотреть на логи вызова метода
    //Сдующий шаг в дебагере это F8
    //Чтобы бысто вызвать метод System.out.println(); нужно после переменной или метода написать"." и sout
    ...
    3. В блоке finally обезательно нужно делать проверки if иначе иногда сложно найти реальную проблему "лекция 38 05:30"*/

    /*13) В StringBuilder / StringBuffer мы продолжаем работать с этим же объектом когда проводим какие-то действия с ним*/
}
